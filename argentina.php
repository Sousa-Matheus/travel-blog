...
$query_rs_argentina = "SELECT cod_pag, titulo, descricao, img1_p FROM tb_blog_lh WHERE cod_pais = 1";
$rs_argentina = mysql_query($query_rs_argentina, $con_lesshome) or die(mysql_error());
$row_rs_argentina = mysql_fetch_assoc($rs_argentina);
$totalRows_rs_argentina = mysql_num_rows($rs_argentina);mysql_select_db($database_con_lesshome, $con_lesshome);
$query_rs_argentina = "SELECT cod_pag, titulo, descricao, img1_p FROM tb_blog_lh WHERE cod_pais = 3";
$rs_argentina = mysql_query($query_rs_argentina, $con_lesshome) or die(mysql_error());
$row_rs_argentina = mysql_fetch_assoc($rs_argentina);
$totalRows_rs_argentina = mysql_num_rows($rs_argentina);
?>
<?php
...
<?php do { ?>
	<div class="col-lg-4"><p><strong><?php echo $row_rs_argentina['titulo']; ?></strong></p>
  <p><img src="<?php echo $row_rs_argentina['img1_p']; ?>" class="img-fluid"> <?php echo $row_rs_argentina['descricao']; ?> <a href="post.php?cod_pag=<?php echo $row_rs_argentina['cod_pag']?>">Saber Mais...</a></p>
		</div>
  <hr>

  <?php } while ($row_rs_argentina = mysql_fetch_assoc($rs_argentina)); ?>
	</div>
</section>
</div>
...
<?php
mysql_free_result($rs_argentina);
?>
